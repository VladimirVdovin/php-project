...
<body style="text-align: center;">
    ...
    if (!empty($_GET['redirect']) && $_GET['redirect'] == true && isset($_SERVER['HTTP_REFERER'])) {
        $referer = strrchr($_SERVER['HTTP_REFERER'], "/");
        if ($referer == "/http.php" && !empty($_SESSION['arr_flash'])) {
    ...
    <div style="background-color: Gainsboro; text-align: center;">
        ...
        <?php
        // Функция getFilePath($path) для нахождения пути к файлу
        define("FILES_EXERCISES", 'files_exercises/');
        function getFilePath($path)
        {
            return FILES_EXERCISES . $path;
        }

        $content1 = intval(file_get_contents(getFilePath('1.txt')));
        $content2 = intval(file_get_contents(getFilePath('2.txt')));
    ...
    <div style="text-align: center;">
        ...
        $arrForSum = [1, 2, 4, 5, 6, 7, 8, 9, 10];
        $sum = array_sum($arrForSum);
    ...
    <div style="text-align: center;">
        ...
        <?php                                              // Номер 1
        echo file_get_contents('dir1/dir2/test.txt') . "<br>";
        ?>

        <?php                                               // Номер 2
        // В файле files_exercises/relative_paths.php находится скрипт
        // С помощью относительного пути читает содержимое dir1/dir2/test.txt
        echo "Первый пример применения относительного пути: ";
        echo "<a href='files_exercises/relative_paths.php'>Ссылка</a><br>";
        ?>

        <?php                                               // Номер 3
        // В файле files_exercises/test/relative_paths.php находится скрипт
        // Он с помощью относительного адреса читает содержимое dir1/dir2/test.txt
        echo "Второй пример применения относительного пути: ";
        echo "<a href='files_exercises/test/relative_paths.php'>Ссылка</a><br>";

        ?>

        <?php                                               // Номер 4
        // В файле files_exercises/test/test/relative_paths.php находится скрипт
        // Он с помощью относительного адреса читает содержимое dir1/dir2/test.txt
        echo "Третий пример применения относительного пути: ";
        echo "<a href='files_exercises/test/test/relative_paths.php'>Ссылка</a><br>";
        ?>
    </div>
    ...
    <div style="background-color: Gainsboro; text-align: center;">
        ...
        <?php
        $path = $_SERVER['DOCUMENT_ROOT'];              // Номер 1
        echo file_get_contents($path . '/dir1/dir2/test.txt') . "<br>";
        ?>

        <?php                                           // Номер 2                                            
        echo file_get_contents($path . '/dir1/dir2/dir3/test.txt') . "<br><hr>";
        ?>
    </div>
    ...
    <div style="text-align: center;">
        ...
        if (file_exists($path) && !file_exists("dir1/sum1.txt")) {
            for ($i = 1; $i <= 5; $i++) {
    ...
    <div style="background-color: Gainsboro; text-align: center;">
        ...
        if (is_dir($path . 'dir3')) {
            ...
            echo "Папка dir3 перенесена в папку dir";
        } elseif (is_dir($path . 'dir/dir3')) {
            rename($path . 'dir/dir3', $path . 'dir3');
            echo "Папка dir3 перенесена обратно в каталог file_exercises";
        }
        ?>
    </div>
    ...
    <div style="background-color: Gainsboro; text-align: center;">
        ...
        $resGetWeekday = getWeekday('files_exercises/dir/weekdays/weekdays.php');
        echo $resGetWeekday;
        ?>
    </div>
    ...
    <div style="text-align: center;">
        ...
        Вставка из файла из предыдущего задания
        <br>
        <?php echo $resGetWeekday; ?>
    </div>
    ...
    <div style="background-color: Gainsboro; text-align: center;">
        ...
        print_r(calcMinusPlus([1, 2, 3, 4, 5]));
