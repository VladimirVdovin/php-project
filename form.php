...
<body style="text-align: center;">
	...
	<!-- ------- Глава одиннадцатая - Основы работы с формами -------- -->
	...
	<div style="text-align: center;">
		...
		<div style="background-color: Gainsboro; text-align: center;">
			...
			<?php
			if (!empty($_GET) && (isset($_GET['name5']))) {
		...
		<?php
		if (!empty($_GET["year1"])) {
			$leapYear = strtotime($_GET["year1"] . "-01-01");
			if (date('L', $leapYear)) {
		...
		<div style="background-color: Gainsboro; text-align: center;">
			...
			<form method="GET">
				...
				<input id="year2" name="year2" value="<?=
														$_GET['year2'] ?? date('Y', $currentTime) ?>">
				<label for="month2">Месяц</label>
				<input id="month2" name="month2" value="<?=
														$_GET["month2"] ?? date("m", $currentTime) ?>">
				<label for="day2">День</label>
				<input id="day2" name="day2" value="<?=
													$_GET['day2'] ?? date('d', $currentTime) ?>">
				<input type="submit">
			</form>

			<?php
			if (!empty($_GET) && isset($_GET['year2'])) {
				$userTime = strtotime($_GET['year2'] . '-' . $_GET["month2"] . '-' . $_GET['day2']);
				echo "До нового года осталось " . (365 - date('z', $userTime)) . " дней";
			} else {
				echo "До нового года осталось " . (365 - date('z', $currentTime)) . " дней";
			}
			?>
		</div>
		...
		<div style="text-align: center;">
			...
			<?php
			if (!empty($_GET) && isset($_GET['name4'])) {
				if (isset($_GET['flag'])) {
		...
		<div style="text-align: center;">
			...
			<form action="" method="GET">
				...
				<input type="checkbox" name="flag3" value="1" <?php if (!empty($_GET['flag3'])) echo 'checked' ?>>
				<input type="checkbox" name="flag4" value="1" <?php if (!empty($_GET['flag4'])) echo 'checked' ?>>
				<input type="checkbox" name="flag5" value="1" <?php if (!empty($_GET['flag5'])) echo 'checked' ?>>
				<input type="submit">
			</form>
		</div>
		...
		<div style="background-color: Gainsboro; text-align: center;">
			...
			<?php
			if (!empty($_GET) && isset($_GET["radio"])) {
				if ($_GET["radio"] === "1") {
					echo "Вы выбрали Ж";
				} elseif ($_GET["radio"] === "2") {
					echo "Вы выбрали М";
		...
		<div style='text-align: center;'>
			...
			<?php
			if (!empty($_GET) && isset($_GET["radio1"])) {
				if ($_GET['radio1'] === "1") {
		...
		<div style="background-color: Gainsboro; text-align: center;">
			...
			<?php
			if (isset($_GET) && !empty($_GET["select"])) {
		...
		<div style="background-color: Gainsboro; text-align: center;">
			...
			<form action="" method="GET">
				<select name="select2">
					<option value="русский" <?php
											if (!empty($_GET) && isset($_GET["select2"]) && $_GET["select2"] === "русский") {
												echo "selected";
											} ?>>Русский</option>
					<option value="сербский" <?php
												if (!empty($_GET) && isset($_GET["select2"]) && $_GET["select2"] === "сербский") {
													echo "selected";
												} ?>>Сербский</option>
					<option value="английский" <?php
												if (!empty($_GET) && isset($_GET["select2"]) && $_GET["select2"] === "английский") {
													echo "selected";
												} ?>>Английский</option>
				</select>
				<input type="submit">
			...
			<?php
			if (isset($_GET) && !empty($_GET["select2"])) {
		...
		<div style="text-align: center;">
			...
			<div style="background-color: Gainsboro; text-align: center;">
				...
				<?php
				if (!empty($_GET) && isset($_GET["letter"])) {
					echo '<br>';
					echo ($_GET["letter"]);
				}
				?>
			...
			<div style="text-align: center;">
				...
				<div style="border: 2px solid lightgray; padding: 10px; display: inline-block;">
					...
					<?php
					if (isset($_GET['nmbr1']) && isset($_GET['nmbr2'])) {
						...
						$arrDivisors = [];
						for ($i = 1; $i <= $maxNmbr; $i++) {
							if ($_GET['nmbr1'] %  $i == 0 && $_GET['nmbr2'] %  $i == 0) {
								$arrDivisors[] = $i;
							}
						}
				...
				<?php
				if (!empty($_GET) && isset($_GET['birthday2'])) {
			...
			<div style="border: 2px solid lightgray; padding: 10px; display: inline-block;">
				...
				<?php
				if (!empty($_GET) && isset($_GET['amount'])) {
			...
			<div style="padding: 10px; display: inline-block;">
				...
				<form method="GET">
					<textarea cols=50 rows=4 name='percent'><?= $_GET['percent'] ?? ''; ?></textarea>
					<br>
					<input type="submit">
				</form>

				<?php
				if (!empty($_GET) && isset($_GET['percent'])) {
					$arr = [];
					$text = str_split($_GET['percent']);
					foreach ($text as $elem) {
						if (!isset($arr[$elem]) && $elem != ' ') {
							$arr[$elem] = 1;
						} elseif ($elem != ' ') {
							$arr[$elem] += 1;
						}
